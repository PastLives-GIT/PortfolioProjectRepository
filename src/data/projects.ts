...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description: 'A federated transfer learning system based on CTrans architecture for training multiplicative error models without direct data transfer. Designed the core algorithm, implemented the estimator, and set up a multi-machine cluster. Feasibility experiments completed; theoretical analysis currently in progress.',
    descriptionZh: '基于 CTrans 架构搭建的不直接传输数据的联邦迁移学习系统，用于训练乘性误差模型。已完成核心算法设计、估计器代码实现与多机群组建，可行性实验验证通过，理论性质证明正在进行中。',
